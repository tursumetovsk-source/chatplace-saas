...
| Channels | Instagram, TikTok, Telegram bots and Telegram Business | Telegram Bot API, Instagram Graph API text path and WhatsApp Cloud API text path (token validation, signed webhook, persisted conversations and replies); TikTok remains planned | Partial |
...
9. AI provider, conversation memory, RAG knowledge base, testing, correction feedback and human handoff. **Implemented for Telegram, Instagram and WhatsApp text paths; operator corrections are persisted, audited and applied to subsequent test/automation prompts; production credentials and live smoke test remain.**
