...
const demoMonitoring: MonitoringData = {
  queue: { PROCESSED: 268, PENDING: 2 }, runs24h: { COMPLETED: 84, WAITING: 3, FAILED: 1 }, recentFailures: [],
  auditLogs: [
...
export default function AnalyticsPage() {
  ...
  const failed24h = monitoring.runs24h.FAILED || 0;
  ...
  return (
    <div className="space-y-7 text-[#0C0C0C]">
      ...
      <section className="rounded-[26px] border border-[#E4E6EB] bg-white p-6 shadow-subtle sm:p-7"><div className="flex items-start justify-between"><div><span className="text-[11px] font-extrabold uppercase tracking-[0.14em] text-[#1E5CFB]">Эксплуатация</span><h2 className="mt-1 text-2xl font-extrabold tracking-[-0.035em]">Состояние автоматизаций</h2></div><Activity className="h-6 w-6 text-[#1E5CFB]" /></div><div className="mt-6 grid gap-4 sm:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-5"><CheckCircle2 className="h-5 w-5 text-emerald-700" /><strong className="mt-3 block text-2xl text-emerald-900">{monitoring.runs24h.COMPLETED || 0}</strong><p className="mt-1 text-sm font-bold text-emerald-800">Завершено за 24 часа</p></div><div className="rounded-2xl bg-amber-50 p-5"><Clock3 className="h-5 w-5 text-amber-700" /><strong className="mt-3 block text-2xl text-amber-900">{queueWaiting}</strong><p className="mt-1 text-sm font-bold text-amber-800">В очереди или на повторе</p></div><div className={`rounded-2xl p-5 ${failed24h ? 'bg-red-50' : 'bg-[#F7F8FB]'}`}><XCircle className={`h-5 w-5 ${failed24h ? 'text-red-700' : 'text-[#777A82]'}`} /><strong className="mt-3 block text-2xl">{failed24h}</strong><p className="mt-1 text-sm font-bold text-[#686B73]">Ошибок за 24 часа</p></div></div>{monitoring.recentFailures.length > 0 && <div className="mt-4 space-y-2">{monitoring.recentFailures.map(failure => <div key={failure.id} className="rounded-xl border border-red-100 bg-red-50 px-4 py-3 text-sm"><strong className="text-red-800">Сценарий остановлен</strong><p className="mt-1 truncate text-xs text-red-700" title={failure.error || ''}>{failure.error || 'Неизвестная ошибка'}</p></div>)}</div>}</section>
